...
class Trainer(LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        for dataname_idx in all_dataname_tensor.unique():
            ...
            embeddings = all_output_tensor[all_dataname_tensor == dataname_idx].numpy()
    ...
    def gather_outputs(self, outputs):
        ...
        all_output_tensor = torch.cat([out['output'] for out in outputs_list], axis=0)
        all_norm_tensor = torch.cat([out['norm'] for out in outputs_list], axis=0)
        all_target_tensor = torch.cat([out['target'] for out in outputs_list], axis=0)
        all_dataname_tensor = torch.cat([out['dataname'] for out in outputs_list], axis=0)
        all_image_index = torch.cat([out['image_index'] for out in outputs_list], axis=0)
